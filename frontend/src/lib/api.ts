...
export const tradeApi = {
  ...
  getTransactions: async (cursor?: number, limit: number = 20): Promise<TransactionsResponse> => {
    const params = new URLSearchParams();
    if (cursor) params.append('cursor', cursor.toString());
    params.append('limit', limit.toString());
    const response = await apiClient.get<TransactionsResponse>(`/trade/transactions?${params.toString()}`);
    return response.data;
  },
};
